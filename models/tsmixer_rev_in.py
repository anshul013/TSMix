...
def build_model(
  ...
  if target_slice:
    x = x[:, :, target_slice]
  x = layers.Lambda(lambda t: tf.transpose(t, perm=[0, 2, 1]))(x) # [Batch, Channel, Input Length]
  x = layers.Dense(pred_len)(x)  # [Batch, Channel, Output Length]
  outputs = layers.Lambda(lambda t: tf.transpose(t, perm=[0, 2, 1]))(x) # [Batch, Output Length, Channel])
